...
# Script para implantar o Tarefo AI no Google Cloud Run usando Cloud Build
# Este script não exige Docker instalado localmente
...
# Construir e enviar a imagem usando Cloud Build
echo "🏗️ Construindo imagem com Cloud Build (não precisa de Docker local)..."
gcloud builds submit --tag ${IMAGE_NAME} .

# Verificar se a construção foi bem-sucedida
if [ $? -ne 0 ]; then
    echo "❌ Falha na construção da imagem. Verifique os logs acima."
    exit 1
fi
